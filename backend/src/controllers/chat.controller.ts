...
export const sendMessage = async (
  ...
  try {
    ...
    try {
      ...
      assistantContent =
        aiResponse.data.response || "I could not generate a response.";
      logger.info(
        `AI response — intent: ${aiResponse.data.intent}, used_rag: ${aiResponse.data.used_rag}`,
      );
    } catch (err) {
    ...
    if (msgCount.rows[0].count <= 2) {
      ...
      await pool.query("UPDATE chat_sessions SET title = $1 WHERE id = $2", [
        title,
        id,
      ]);
    }
